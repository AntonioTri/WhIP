...
class GameSceneScript: SKScene, SKPhysicsContactDelegate{
    ...
    var blinkingRedSignal: BlinkingRedSignal!
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        // Il segnale cannotTrow è un segnale di kill, disattiva tutti i tipi
        // Di interazione con il sistema di lancio
        if viewModel.cannotTrow == 1{
            viewModel.canTrow = 0
            viewModel.canTrowSecondSignal = 0
            viewModel.cannotTrow = 0
            setSemaferoRed = true
        }
        ...
        // Si controlla se deve essere mostrato il segnale rosso
        self.blinkSignal()
    ...
    private func checkVictoryCondition(){
        ...
        if condizioneVittoria == 0 || condizioneVittoria == 1 || condizioneVittoria == 2 {
            ...
            case 0:
                ...
                self.simulation.forceSimulationEnd()
    ...
    func initialize(){
        ...
        self.blinkingRedSignal = BlinkingRedSignal(scene: self)
    ...
    func setViewModel(viewModel: ViewModel ){
        ...
        self.latencyModuler = LatencyModuler(viewModel: self.viewModel)
    }
    
    private func blinkSignal() {
        
        if viewModel.frDurability < 35 && autolockBlinking {
            self.autolockBlinking = false
            self.blinkingRedSignal.startBlinking()
        }
        
        if (viewModel.frDurability > 50 && !autolockBlinking) || condizioneVittoria >= 0 {
            self.autolockBlinking = true
            self.blinkingRedSignal.stopBlinking()
        }
    }
}
