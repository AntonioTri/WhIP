...
class FishCollection{
    ...
    func scrollPage(touch: UITouch){
        ...
        if touchedNode == trophies {
            ...
            self.scene.viewModel.sendMessage(key: "cannotTrow", value: 1)
            self.scene.latencyModuler.moveSignalUP()
        ...
        for n in startIndex...endingIndex {
            ...
            for m in 0...3 {
                ...
                if savingCenter.getSavedBool(key: currentName) {
                    ...
                    currentTrophy.texture = SKTexture(imageNamed: "questionMark")
                } else {
    ...
    private func turnBackToPreviousState(){
        ...
        if scene.getCurrentState() == 0 {
            ...
            self.scene.menu.run(SKAction.group([SKAction.moveTo(x: 13.842, duration: 0.8), fadeIn]))
            self.collectionNode.run(removeSequence)
            self.scene.latencyModuler.moveSignalCenter()
            self.scene.viewModel.sendMessage(key: "cannotTrow", value: 1)
